...
| R-20 | Ensure PDF audiobook import, processing, errors, resume, cancellation, and optional Gemini cleanup are solid. | Partial: local-first default and Gemini-only-on-explicit-choice are covered by focused backend tests. Fixture-driven app import, cancellation, resume, corrupt-PDF, and user-visible error passes remain. |
...
| --- | --- | --- | --- | --- |
| VQ-001 | First launch | A brand-new install lands in onboarding, never a blank player or hidden sheet. | Fresh-state run plus onboarding unit/UI coverage. | Partial: exact local Release build showed onboarding first and coordinator tests pass. The final downloaded DMG still needs the same evidence. |
...
| VQ-008 | Audiobooks | PDF import, validation, progress, cancel, retry, resume, and corrupt-PDF recovery are clear and safe. | Fixture-driven tests and manual import. | Partial: local-first and Gemini-explicit backend behavior is covered; PDF fixture/UI proof is open. |
...
| VQ-044 | First-launch cleanup | An interrupted backend extraction cannot accumulate stale temporary directories or delete a fresh concurrent extraction. | Deterministic old/new directory test. | Done locally: cleanup deletes only an hour-old Voqora staging directory; 64 Swift tests pass. |
...
| Backend tests | `make test-backend`: 180 passed | Local speech and API logic pass headlessly | Packaged app/backend wiring and actual model UX | Exact candidate launch and manual speech pass |
